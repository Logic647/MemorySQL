...
- **腾讯 WorkBuddy 适配**(2026-09-22,未发版):`capture-workbuddy` 捕获 `~/.workbuddy/projects/**/*.jsonl` + `workbuddy.db` 元数据增强(title/cwd/created_at);watcher 覆盖 jsonl/db;MCP 连接器写 `~/.workbuddy/mcp.json` 的 `mcpServers.memorysql`。本机未装,合成样本单测 5 用例;typecheck 0 / vitest **125:125**。真机验收待装机
- **Qoder CLI / CN 适配**(2026-09-23,未发版):`capture-qoder` 双根 `~/.qoder` + `~/.qoder-cn` 的 `projects/**/*.jsonl`(Claude 兼容)+ `state.json` 补 title/cwd;`watchPaths` 工厂签名改为收 `sourceRoot`;MCP 连接器写 `settings.json`。调研:iFlow 已停服(待办作废)、Comate Zulu 高优先待做。typecheck 0 / vitest **126:126**
- 精确进度:见 `docs/DEVLOG.md` 最新一条(顶部);下一步:**真机验收 v0.5.3 自动更新与 WorkBuddy/Qoder 适配(装机后)**、用户过目截图 → 按 `docs/promo/checklist.md` 发布宣传(V2EX 首发等)、winget bot 跟进(0.5.0 版 PR)、WorkBuddy+Qoder 适配随下一版发版、评估 Comate Zulu
